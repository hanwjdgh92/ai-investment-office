...
- [x] **1단계 — 정보 수집 & 추천 리포트** (완료)
  ...
  리포트를 생성합니다. 크립토 데스크는 리서치 종합 전에 Bull(강세 논거)/Bear(약세 논거·반박)가 먼저
  논쟁을 벌입니다. (15명 — 2단계 크립토 파일럿까지 합치면 총 19명입니다)
...
뉴스·심리 3가지 관점으로 분석을 나눠 맡습니다. 국내주식/해외주식 데스크는 리서치 종합 담당(Anchor/
Compass)이 강세·약세 논거를 스스로 세운 뒤 저울질해 리서치 노트로 정리합니다. 크립토 데스크는 Bull(강세
논거 작성)과 Bear(약세 논거 작성·Bull 반박)가 먼저 논쟁을 벌이고, Node는 그 결과를 심판(저울질)만
합니다 — Node 스스로 새 논거를 만들지 않습니다.
...
| RESEARCH(크립토) | Bull | 크립토 강세 논거 작성 | `.claude/agents/bull.md` |
| RESEARCH(크립토) | Bear | 크립토 약세 논거 작성·Bull 반박 | `.claude/agents/bear.md` |
| RESEARCH(크립토) | Node | 크립토 리서치 심판(저울질) | `.claude/agents/node.md` |
